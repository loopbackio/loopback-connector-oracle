...
function mixinMigration(Oracle) {
  Oracle.prototype.showFields = function(model, cb) {
    const sql_table_name_filter = ' WHERE table_name=\'' + this.table(model) + '\'';

    const owner = this.schema(model);
    const sql_owner_filter = owner !== '' ? ' AND owner=\'' + owner + '\'' : '';
    const sql_tab_columns_table = owner !== '' ? 'ALL' : 'USER';
    ...
    const sql = 'SELECT column_name AS "column", data_type AS "type",' +
      ...
      ' FROM "SYS"."' + sql_tab_columns_table + '_TAB_COLUMNS"' +
      sql_table_name_filter + sql_owner_filter;
