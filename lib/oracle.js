 ...
 */
function executeSQL(self, sql, params, callback) {
 ...
 *
 * @param self The adapter instance
 * @param sql The SQL statement
 * @param params An array of parameters
 *
 */
function executeSQLSync(self, sql, params) {
    if (self.settings.debug) {
        if (params && params.length > 0) {
            self.debug('SQL: ' + sql + '\nParameters: ' + params);
        } else {
    ...
    var data = self.connection.executeSync(sql, params);
    if (self.settings.debug && data) self.debug("%j", data);
    return data;
}
...
Oracle.prototype.querySync = function (sql, params) {
    ...
    if (!this.connection) {
        this.connection = oracle.connectSync(this.settings);
    }
    return executeSQLSync(this, sql, params);
}
