...
Oracle.prototype.query = function (sql, params, callback) {
  ...
  if(self.schema.connected) {
    ...
    function(cb) {              
      if (!self.schema.connected) {
        self.debug('Connecting to ' + self.settings.hostname);
    ...
    function(err, data) {
      if(data) {
        // Remove the undefined result from the connect
        data.shift();
        // Set the 2nd element which is an array
        data = data.shift(); 
      }
      callback(err, data);
    });
...
Oracle.prototype.updateOrCreate = function (model, data, callback) {
    ...
    sql += ' SET ' + combined;
    if(data.id) {
      sql += ' WHERE ' + self.idColumnEscaped(model) +' = ' + data.id + ';';
    } else {
      sql += ' WHERE ' + self.idColumnEscaped(model) +' IS NULL' + ';';
    }
...
Oracle.prototype.save = function (model, data, callback) {
  ...
  if(data.id) {
    sql += ' WHERE ' + this.idColumnEscaped(model) +' = ' + data.id;
  } else {
    sql += ' WHERE ' + this.idColumnEscaped(model) +' IS NULL';
  }
...
Oracle.prototype.getColumns = function(model){
    ...
    var self = this;
    var names = Object.keys(cols).map(function(c) {
 ...
 * Discover the models
 */
Oracle.prototype.discoverModels = function (options, cb) {
    ...
    var calls = [function (callback) {
        if (options.all) {
            self.query(paginateSQL('SELECT \'table\' AS "type", table_name AS "name", owner AS "owner"'
                + ' FROM all_tables', 'owner, table_name', options), callback);
        } else {
            self.query(paginateSQL('SELECT \'table\' AS "type", table_name AS "name",'
                + ' SYS_CONTEXT(\'USERENV\', \'SESSION_USER\') AS "owner" FROM user_tables',
                'table_name', options), callback);
        }
    }];
    ...
    if (options.views) {
        calls.push(function (callback) {
            if (options.all) {
                self.query(paginateSQL('SELECT \'view\' AS "type", view_name AS "name",'
                    + ' owner AS "owner" FROM all_views',
                    'owner, view_name', options), callback);
            } else {
                self.query(paginateSQL('SELECT \'view\' AS "type", view_name AS "name",'
                    + ' SYS_CONTEXT(\'USERENV\', \'SESSION_USER\') AS "owner" FROM user_views',
                    'view_name', options), callback);
            }
        });
    }
...
Oracle.prototype.discoverModelProperties = function (owner, table, cb) {
    ...
    if (owner) {
        this.query(paginateSQL('SELECT owner AS "owner", table_name AS "tableName", column_name AS "columnName", data_type AS "dataType",'
            + ' data_length AS "dataLength", nullable AS "nullable"'
            + ' FROM all_tab_columns'
            + ' WHERE owner=\''+owner+'\''
            + (table ? ' AND table_name=\'' + table + '\'' : ''),
            'table_name, column_id', {}), callback);
    } else {
        this.query(paginateSQL('SELECT SYS_CONTEXT(\'USERENV\', \'SESSION_USER\') AS "owner", table_name AS "tableName", column_name AS "columnName", data_type AS "dataType",'
            + ' data_length AS "dataLength", nullable AS "nullable"'
...
Oracle.prototype.discoverPrimaryKeys= function(owner, table, cb) {
...
Oracle.prototype.discoverForeignKeys= function(owner, table, cb) {
    var sql =
...
function getColumnsToAdd(model, actualFields){
    ...
    propNames.forEach(function (propName) {
        ...
        var found = searchForPropertyInActual.call(self, self.column(model, propName), actualFields);
...
function addPropertyToActual(model, propName){
    ...
    var sqlCommand = 'ADD ' + self.columnEscaped(model, propName)
        + ' ' + self.columnDataType(model, p) + " " + (propertyCanBeNull.call(self, model, propName) ? "" : " NOT NULL");
    return sqlCommand;
};
...
function getPropertiesToModify(model, actualFields){
    ...
    function datatypeChanged(propName, oldSettings){
        ...
        return oldSettings.type.toUpperCase() !== self.columnDataType(m, propName);
    };
...
function modifyDatatypeInActual(model, propName) {
    var self = this;
    var sqlCommand = 'MODIFY ' + self.columnEscaped(model, propName) + ' ' + self.columnDataType(model, propName);
    return sqlCommand;
};

function modifyNullabilityInActual(model, propName) {
    var self = this;
    var sqlCommand = 'MODIFY ' + self.columnEscaped(model, propName)+ ' ';
...
function getColumnsToDrop(model, actualFields){
    ...
    actualFields.forEach(function (actualField) {
        if (self.idColumn(model) === actualField.column) return;
        if (actualFieldNotPresentInModel(actualField, model)) {
            sql.push('DROP ' + this.escapeName(actualField.column));
        }
    });
    return sql;
...
Oracle.prototype.dropTable = function (model, cb) {
    ...
    var triggerName = self.escapeName(model + '_ID_SEQUENCE');
        ...
        self.query('DROP TABLE ' + name, function (err, data) {
            if (err && err.toString().indexOf('ORA-00054') >= 0) {
                ...
                if (count < 3) {
                    ...
                    setTimeout(dropTableFun, 100 * count);
                    return;
                }
            }
...
Oracle.prototype.createTable = function (model, cb) {
  ...
  async.series([
    ...
    function(callback) {
        self.query('CREATE OR REPLACE TRIGGER ' + self.escapeName(model +'_ID_TRIGGER') +
            ' BEFORE INSERT ON ' + name +  ' FOR EACH ROW \n' + 
            'BEGIN\n' +
            '  SELECT '+ triggerName +'.NEXTVAL INTO :new.'+ self.idColumnEscaped(model) +' FROM dual;\n' +
            'END;', callback);
    }],
    cb);      
};
...
Oracle.prototype.columnDataType = function (model, p) {
    var columnMetadata = this.columnMetadata(model, p);
    var colType = columnMetadata && columnMetadata.dataType;
    var colLength = columnMetadata && columnMetadata.dataLength || this._models[model].properties[p].length;
    if (colType) {
        return colType + (colLength ? '(' + colLength + ')' : '');
    }
    var prop = this._models[model].properties[p];
    if(!prop) {
        return null;
    }
