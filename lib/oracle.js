...
function dateToOracle(val, dateOnly) {
  ...
  var offset = moment().utcOffset();  // Get the offset for the configured timezone on this app server.
  var localMomentDate = moment(val.getTime());
  localMomentDate.utcOffset(offset);   // Reset to the same offset.  For values in the daylight savings range, this was getting off by an hour.

  // Now get a date string with the local time.
  var dateStr = [
    localMomentDate.year(),
    fz(localMomentDate.month() + 1),
    fz(localMomentDate.date()),
  ].join('-') + ' ' + [
    fz(localMomentDate.hour()),
    fz(localMomentDate.minute()),
    fz(localMomentDate.second()),
  ].join(':');

  if (!dateOnly) {
    dateStr += '.' + ms(localMomentDate.millisecond());
  }
