...
function addPropertyToActual(model, propName){
    ...
    var sqlCommand = 'ADD ' + self.columnEscaped(model, propName)
        + ' ' + datatype(p) + " " + (propertyCanBeNull.call(self, model, propName) ? "" : " NOT NULL");
    return sqlCommand;
};
...
function modifyDatatypeInActual(model, propName) {
    var self = this;
    var sqlCommand = 'MODIFY ' + self.columnEscaped(model, propName) + ' ' + datatype(self._models[model].properties[propName]);
    return sqlCommand;
};
