...
Oracle.prototype.buildExpression = function(columnName, operator, operatorValue,
    propertyDefinition) {
  switch(operator) {
    case 'like':
    ...
    case 'regexp':
      if (operatorValue.global)
        console.warn('Oracle regex syntax does not respect the `g` flag');
      ...
      var query = ' REGEXP_LIKE (' + columnName + ', ?';
      var hasFlags = flags.length !== 0;
      if (hasFlags)
        query += ", '" + flags.join('') + "'";
      query += ')';

      return new ParameterizedSQL(query, [operatorValue.source]);
    default:
