...
Oracle.prototype.discoverModelProperties = function(options, cb) {
  if(!cb && typeof options ==='function') {
    cb = options;
    options = {};
  }
  ...
  this.query(paginateSQL('SELECT SYS_CONTEXT(\'USERENV\', \'SESSION_USER\') AS "owner", table_name AS "tableName", column_name AS "columnName", data_type AS "dataType",' 
            ...
            + (options.model? ' WHERE table_name=\'' + options.model + '\'': ''),
            'table_name, column_id', options), function(err, results) {
    if(err) {
      cb(err, results);
    } else {
      results.map(function(r) {
        r.dataType = oracleDataTypeToJSONType(r.dataType, r.dataLength);
      });
      cb(err, results);
    }
  });
...
Oracle.prototype.discoverPrimaryKeys= function(owner, table, cb) {
     ...
     var sql = 'SELECT uc.owner AS TABLE_SCHEM, uc.constraint_name AS PK_NAME,'
     + 'uc.table_name, col.column_name, col.position AS KEY_SEQ'
     + ' FROM USER_CONSTRAINTS uc, USER_CONS_COLUMNS col'
...
Oracle.prototype.discoverForeignKeys= function(owner, table, cb) {
    var sql =
     'SELECT a.constraint_name AS PK_NAME, a.table_name AS PKTABLE_NAME, a.column_name AS PKCOLUMN_NAME, a.position AS KEY_SEQ,'
     + ' jcol.owner AS FKTABLE_SCHEM, jcol.constraint_name AS FK_NAME,'
     + ' jcol.table_name AS FKTABLE_NAME, jcol.column_name AS FKCOLUMN_NAME'
     + ' FROM'
     + ' (SELECT'
     + ' uc.table_name, uc.constraint_name, uc.r_constraint_name, col.column_name, col.position'
     + ' FROM'
...
function addPropertyToActual(model, propName){
    ...
    var sqlCommand = 'ADD "' + propName + '" ' + datatype(p) + " " + (propertyCanBeNull.call(self, model, propName) ? "" : " NOT NULL");
    return sqlCommand;
};
...
function modifyDatatypeInActual(model, propName) {
    var self = this;
    var sqlCommand = 'MODIFY "' + propName + '" ' + datatype(self._models[model].properties[propName]);
    return sqlCommand;
};

function modifyNullabilityInActual(model, propName) {
    var self = this;
    var sqlCommand = 'MODIFY "' + propName + '" ';
...
function getColumnsToDrop(model, actualFields){
    ...
    actualFields.forEach(function (actualField) {
        ...
        if (actualFieldNotPresentInModel(actualField, model)) {
            sql.push('DROP "' + actualField.Field + '"');
        }
    });
    return sql;
