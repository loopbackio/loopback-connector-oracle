...
Oracle.prototype.toFilter = function (model, filter) {
    ...
    if (filter.where) {
        ...
        if(!filter.order) {
          // No ORDER BY, merge the ROWNUM conditions to WHERE
          for(var i=0; i<pagination.length; i++) {
            fields.push(pagination[i].replace('R', 'ROWNUM'));
          }
        }
        if (fields.length) {
    ...
    if (filter.order) {
        ...
        if(pagination.length) {
          ...
          out = '(SELECT ROW_NUMBER() OVER (' + orderBy + ') R, ' + this.getColumns(model) +' FROM ' + this.tableEscaped(model) + out + ') WHERE ' + pagination.join(' AND ');
        } else {
