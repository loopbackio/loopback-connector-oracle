...
Oracle.prototype.executeSQLSync = function (sql, params) {
  ...
  if (!self.pool) {
    self.pool = self.driver.createConnectionPoolSync(self.settings);
    if (self.settings.debug) {
      debugConnection('Connection acquired: ', self.pool.getInfo());
    }
  }
...
Oracle.prototype.destroyAll = function destroyAll(model, where, callback) {
  ...
  this.query('DELETE  FROM '
    + ' ' + this.toFilter(model, where && {where: where}), function (err, info) {
    callback && callback(err, {count: info && info.updateCount});
  }.bind(this));
};
...
Oracle.prototype.update = Oracle.prototype.updateAll = function (model, where, data, callback) {
  var fields = this.toFields(model, data);
  var sql = 'UPDATE ' + this.tableEscaped(model) + ' SET ' + fields.sql;

  sql += this.buildWhere(model, where);
...
Oracle.prototype.toFields = function (model, data, forCreate) {
  ...
  if (forCreate) {
      ...
      if (props[key]) {
        if (!self.id(model, key)) {
          if (props[key].type === String || props[key].type === Buffer) {
            ...
            columns.push(self.columnEscaped(model, key));
            fields.push(self.toDatabase(props[key], data[key]));
          }
        } else {
          if (data[key]) {
            if (props[key].type === String || props[key].type === Buffer) {
    ...
    if (columns.length > 0) {
      ...
      var idName = self.columnEscaped(model, self.idName(model));
      result.sql = '(' + idName + ') VALUES (default)';
    }
    ...
    Object.keys(data).forEach(function (key) {
      if (props[key]) {
        if (props[key].type === String || props[key].type === Buffer) {
          ...
          columns.push(self.columnEscaped(model, key) + '= ' + self.toDatabase(props[key], data[key]));
        }
      }
    });
...
Oracle.prototype.schemaName = function (model) {
  // Check if there is a 'schema' property for mssql
  var dbMeta = this._models[model].settings && this._models[model].settings.oracle;
...
Oracle.prototype.tableEscaped = function (model) {
  var schemaName = this.schemaName(model);
  if (schemaName && schemaName !== this.settings.user) {
    return this.escapeName(schemaName) + '.' + this.escapeName(this.table(model));
  } else {
...
Oracle.prototype.buildWhere = function (model, conds) {
...
Oracle.prototype._buildWhere = function (model, where) {
  ...
  if (typeof conds === 'string') {
    ...
    Object.keys(conds).forEach(function (key) {
      ...
      var sqlCond = null;
      if (where[key] && where[key].constructor.name === 'RegExp') {
        var regex = where[key];
      ...
      if (props[key]) {
        ...
        if (conds[key].constructor.name === 'Object') {
          ...
          var val = conds[key][operator];
          if (operator === 'between') {
            filterValue = self.toDatabase(props[key], val[0]) + ' AND ' +
              self.toDatabase(props[key], val[1]);
          } else if (operator === 'inq' || operator === 'nin') {
            for (var i = 0; i < val.length; i++) {
              val[i] = self.toDatabase(props[key], val[i]);
            }
            filterValue = val.join(',');
          } else {
            filterValue = self.toDatabase(props[key], val);
          }
          sqlCond = self.columnEscaped(model, key);
          if ((operator === 'inq' || operator === 'nin') && filterValue.length === 0) {
            fields.push(operator === 'inq' ? '1 = 2' : '1 = 1');
            return true;
          }
          switch (operator) {
            case 'gt':
          ...
          sqlCond += (operator === 'inq' || operator === 'nin') ?
