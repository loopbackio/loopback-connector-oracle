...
function executeSQL(self, sql, params, callback) {
    ...
    self.connection.execute(sql, params, function (err, data) {
        if(err && err.toString().indexOf('ORA-03114') >= 0) {
            ...
            self.schema.connected = self.schema.connecting = false;
        }
...
function executeSQLSync(self, sql, params) {
    ...
    try {
        ...
        if(err && err.toString().indexOf('ORA-03114') >= 0) {
            self.connection = null; // Remove the connection so that it will be reconnected
            this.schema.connected = this.schema.connecting = false;
        }
    }
}
 ...
 */
Oracle.prototype.discoverModelsSync = function (options) {
    options = options || {};
...
Oracle.prototype.discoverModelPropertiesSync = function (owner, table) {
    var sql = queryColumns(owner, table);
 ...
 * @returns {*}
 */
Oracle.prototype.discoverPrimaryKeysSync= function(owner, table) {
    var sql = queryForPrimaryKeys(owner, table);
 ...
 * @returns {*}
 */
Oracle.prototype.discoverForeignKeysSync= function(owner, table) {
    var sql = queryForeignKeys(owner, table);
    return this.querySync(sql);
}
...
Oracle.prototype.discoverExportedForeignKeys= function(owner, table, cb) {
 ...
 * @returns {*}
 */
Oracle.prototype.discoverExportedForeignKeysSync= function(owner, table) {
    var sql = queryExportedForeignKeys(owner, table);
    return this.querySync(sql);
}
